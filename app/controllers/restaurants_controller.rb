class RestaurantsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @restaurant.update(restaurant_params) and @restaurant.address.update(address_params)
  ...
  def address_params
    params.require(:restaurant).permit(address: [:house_number, :street, :city])[:address]
  end

  def get_address_from_attributes
    address_attributes = address_params
    address = Address.where(house_number: address_attributes[:house_number],
                            street: address_attributes[:street],
                            city: address_attributes[:city]).first

    unless address
      return Address.create(address_params)
    end
    address
  end
end
